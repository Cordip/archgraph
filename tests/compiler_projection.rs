use archgraph::{cli, compiler, config, context, model::{ArchitectureIr, CodeEdge, EdgeOrigin}, projection::{self, EntryKind}, provider::InMemoryProvider, server};
...
fn repository() -> tempfile::TempDir {
    ...
    for path in ["src/api/routes.py", "src/api/handlers/route.py", "src/domain/model.py", "src/persistence/db.py", "src/shared/types.py", "src/main.py"] { write(temp.path(), path); }
    temp
}
fn edge(from: &str, to: &str) -> CodeEdge {
    CodeEdge { from_file: from.into(), to_file: to.into(), kind: "IMPORTS".into(), confidence: Some(0.8), reason: Some("fixture".into()) }
}
fn imports() -> Vec<CodeEdge> {
    vec![edge("src/api/routes.py", "src/domain/model.py"), edge("src/api/handlers/route.py", "src/domain/model.py"),
        edge("src/domain/model.py", "src/persistence/db.py"), edge("src/persistence/db.py", "src/domain/model.py")]
}
async fn compile(root: &Path, edges: Vec<CodeEdge>) -> ArchitectureIr {
    compiler::compile(root, &config::parse(CONFIG).unwrap(), &InMemoryProvider { edges, failure: None }, false).await.unwrap()
}
...
async fn provider_failure_is_not_a_clean_graph() {
    let temp = repository();
    let provider = InMemoryProvider { edges: Vec::new(), failure: Some("index is unavailable".into()) };
    let result = compiler::compile(temp.path(), &config::parse(CONFIG).unwrap(), &provider, false).await;
...
async fn unknown_and_unassigned_provider_paths_are_diagnostics() {
    let temp = repository();
    let ir = compile(temp.path(), vec![edge("outside.py", "src/domain/model.py"), edge("../escape.py", "src/domain/model.py")]).await;
...
async fn focus_projects_deep_edges_and_retains_direct_ownership() {
    ...
    assert!(view.nodes.iter().any(|n| n.entry_kind == EntryKind::DirectFiles));
    let aggregated = view.edges.iter().find(|e| e.edge.from == "node:app.api" && e.edge.to == "node:app.domain").unwrap();
...
async fn cross_boundary_incoming_outgoing_and_manual_edges_survive() {
    ...
    assert!(view.nodes.iter().any(|n| n.entry_kind == EntryKind::File && n.file_path.as_deref() == Some("src/domain/model.py")));
    assert!(view.nodes.iter().any(|n| n.outside_focus && n.architecture_id.as_deref() == Some("app.persistence")));
    assert!(view.edges.iter().any(|e| e.edge.from == "file:src/domain/model.py" && e.edge.to == "node:app.persistence"));
    assert!(view.edges.iter().any(|e| e.edge.from == "node:app.persistence" && e.edge.to == "file:src/domain/model.py"));
    let root = projection::project(&ir, "app", 20).unwrap();
    let manual = root.edges.iter().find(|e| e.edge.from == "boundary:app").unwrap();
    assert_eq!(manual.edge.origin, EdgeOrigin::Manual);
    assert!(manual.edge.evidence.is_empty());
    assert!(root.nodes.iter().any(|n| n.outside_focus && n.architecture_id.as_deref() == Some("external.service")));
}
...
async fn leaf_projection_is_lossless_beyond_aggregated_evidence_cap() {
    ...
    let aggregated = ir.edges.iter().find(|e| e.origin == EdgeOrigin::Observed).unwrap();
...
async fn context_increases_edge_and_violation_evidence_limit_from_raw_imports() {
    ...
    let edge = result.observed_internal.iter().find(|e| e.edge.from == "node:app.domain").unwrap();
    assert_eq!(edge.edge.count, 65);
    assert_eq!(edge.edge.evidence.len(), 50);
    assert!(result.projection.violations.iter().all(|v| v.evidence.len() == 50));
...
async fn manual_edges_do_not_trigger_rules_even_with_imports_kind() {
    ...
    let ir = compiler::compile(temp.path(), &validated, &InMemoryProvider::default(), false).await.unwrap();
...
async fn scoped_check_matches_actual_owners_not_similarly_named_or_unaffected_descendants() {
    let temp = repository();
    let yaml = CONFIG.replace("from: app.domain\n    to: app.persistence", "from: app.api\n    to: app.domain\n    include_descendants: false");
    let provider = InMemoryProvider { edges: vec![edge("src/api/routes.py", "src/domain/model.py"), edge("src/api/handlers/route.py", "src/domain/model.py")], failure: None };
    let ir = compiler::compile(temp.path(), &config::parse(&yaml).unwrap(), &provider, false).await.unwrap();
    assert_eq!(cli::matching_violations(&ir, Some("app.api")).unwrap().len(), 1);
    assert!(cli::matching_violations(&ir, Some("app.api.handlers")).unwrap().is_empty());
    assert!(cli::matching_violations(&ir, Some("missing")).is_err());
    let view = projection::project(&ir, "app.api", 20).unwrap();
    let child_edge = view.edges.iter().find(|e| e.edge.from == "node:app.api.handlers").unwrap();
...
async fn read_only_http_endpoints_share_projection_and_do_not_expose_source_or_cypher() {
    let temp = repository();
    let ir = compile(temp.path(), imports()).await;
    let expected = serde_json::to_value(projection::project(&ir, "app.domain", 20).unwrap()).unwrap();
    let app = server::router(Arc::new(ir));
    let response = app.clone().oneshot(Request::builder().uri("/api/focus/app.domain").body(Body::empty()).unwrap()).await.unwrap();
    ...
    let response = app.clone().oneshot(Request::builder().method("POST").uri("/api/meta").body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(response.status(), StatusCode::METHOD_NOT_ALLOWED);
    let response = app.oneshot(Request::builder().uri("/api/search?q=domain").body(Body::empty()).unwrap()).await.unwrap();
    let bytes = to_bytes(response.into_body(), usize::MAX).await.unwrap();
