...
use std::{os::unix::fs::PermissionsExt, process::{Command, Output}};
...
impl Fixture {
    fn new() -> Self {
        assert!(Command::new("python3").arg("--version").output().is_ok(), "CLI subprocess fixtures require python3; core tests do not");
        let root = tempfile::Builder::new().prefix("archgraph repo ").tempdir().unwrap();
    ...
    fn run(&self, arguments: &[&str], mode: &str) -> Output {
        Command::new(env!("CARGO_BIN_EXE_archgraph")).arg("--root").arg(self.root.path())
            .args(arguments).env("GITNEXUS_BIN", &self.executable)
            .env("ARCHGRAPH_FAKE_MODE", mode).env("ARCHGRAPH_FAKE_LOG", self.root.path().join("provider.log"))
            .output().unwrap()
    }
    fn logs(&self) -> Vec<Value> {
        std::fs::read_to_string(self.root.path().join("provider.log")).unwrap().lines()
...
fn check_exit_zero_for_clean_two_for_violations_and_scope_filtering() {
    ...
    assert_eq!(fixture.run(&["check", "app.a"], "violation").status.code(), Some(2));
    assert_eq!(fixture.run(&["check", "app.c"], "violation").status.code(), Some(0));
    assert_eq!(fixture.run(&["compile"], "violation").status.code(), Some(0));
}

#[test]
fn provider_failures_and_invalid_configuration_exit_one() {
    let fixture = Fixture::new();
    for mode in ["unindexed", "malformed", "query_failure", "bad_count", "bad_confidence"] {
        let output = fixture.run(&["check", "--json"], mode);
        assert_eq!(output.status.code(), Some(1), "mode {mode}");
        assert!(output.stdout.is_empty(), "provider failure emitted success JSON");
    }
    std::fs::write(fixture.root.path().join("architecture.yaml"), "invalid: [yaml").unwrap();
...
fn cli_usage_errors_do_not_collide_with_violation_exit_code() {
    let fixture = Fixture::new();
    assert_eq!(fixture.run(&["check", "--unknown-option"], "clean").status.code(), Some(1));
...
fn json_show_context_and_check_are_machine_readable() {
    let fixture = Fixture::new();
    for arguments in [&["show", "--format", "json"][..], &["context", "app", "--json"][..], &["check", "--json"][..]] {
        let output = fixture.run(arguments, "violation");
        let value: Value = serde_json::from_slice(&output.stdout).unwrap();
        assert!(value.is_object());
    }
    let mermaid = fixture.run(&["show", "--format", "mermaid"], "violation");
    assert!(String::from_utf8(mermaid.stdout).unwrap().starts_with("flowchart LR"));
}
...
fn pagination_uses_argv_cwd_and_a_final_empty_page() {
    ...
    let queries: Vec<_> = logs.iter().filter(|log| log["args"][1].as_str().is_some_and(|q| q.contains(" SKIP "))).collect();
    assert_eq!(queries.len(), 3);
    for (offset, query) in queries.into_iter().enumerate() {
        assert!(query["args"][1].as_str().unwrap().ends_with(&format!("SKIP {offset} LIMIT 1")));
        assert_eq!(query["args"].as_array().unwrap().len(), 2);
        assert_eq!(query["cwd"].as_str().unwrap(), fixture.root.path().canonicalize().unwrap().to_str().unwrap());
    }
}
...
fn reindex_is_incremental_repo_is_optional_and_progress_does_not_corrupt_json() {
    let fixture = Fixture::new();
    let yaml = YAML.replace("page_size: 1", "page_size: 1, repo: 'repo with spaces; data not shell'");
