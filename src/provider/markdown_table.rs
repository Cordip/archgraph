...
fn cells(line: &str) -> Vec<String> {
    ...
    while let Some(c) = chars.next() {
        ...
        match c {
            '\\' => match chars.peek().copied() {
                Some('|') | Some('\\') => { current.push(chars.next().expect("peeked character exists")); }
                _ => current.push('\\'),
            },
            '|' => {
    ...
    cells.push(current.trim().to_owned());
    if line.starts_with('|') { cells.remove(0); }
    if last_was_separator { cells.pop(); }
    cells
}
...
pub fn parse(markdown: &str) -> Result<Table> {
    ...
    if lines.len() < 2 { return Err(compatibility("table requires a header row and separator row (including for zero rows)")); }
    ...
    if unique.len() != headers.len() { return Err(compatibility("duplicate table column headers")); }
    let separators = cells(lines[1]);
    if separators.len() != headers.len() || separators.iter().any(|cell| {
        ...
        return Err(compatibility("invalid Markdown separator row; expected one --- cell per header"));
    }
    let mut rows = Vec::new();
    for (index, line) in lines.iter().enumerate().skip(2) {
        let row = cells(line);
        if line.trim().is_empty() || row.len() != headers.len() {
            return Err(compatibility(format!("table row {} has {} cells; expected {}", index + 1, row.len(), headers.len())));
        }
        rows.push(row);
    }
