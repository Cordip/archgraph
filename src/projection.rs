...
use crate::{config::{immediate_child, is_within, parent_id, Interface, NodeKind}, model::*, rules};
...
pub enum EntryKind { Architecture, File, DirectFiles, Boundary }
...
impl From<&CompiledNode> for NodeSummary {
    fn from(node: &CompiledNode) -> Self {
        Self { id: node.id.clone(), title: node.title.clone(), kind: node.kind,
            description: node.description.clone(), descendant_file_count: node.descendant_file_count,
...
fn architecture_entry(node: &CompiledNode, outside: bool) -> ProjectionNode {
    ProjectionNode { id: format!("node:{}", node.id), title: node.title.clone(), entry_kind: EntryKind::Architecture,
        architecture_id: Some(node.id.clone()), node_kind: Some(node.kind), file_path: None,
        file_count: node.descendant_file_count, description: node.description.clone(), interfaces: node.interfaces.clone(),
        outside_focus: outside, violation_rule_ids: Vec::new() }
}
fn file_entry(path: &str, node: &CompiledNode) -> ProjectionNode {
    ProjectionNode { id: format!("file:{path}"), title: path.into(), entry_kind: EntryKind::File,
        architecture_id: Some(node.id.clone()), node_kind: None, file_path: Some(path.into()), file_count: 1,
        description: None, interfaces: Vec::new(), outside_focus: false, violation_rule_ids: Vec::new() }
}
fn direct_entry(focus: &CompiledNode) -> ProjectionNode {
    ProjectionNode { id: format!("direct:{}", focus.id), title: "Directly owned files".into(), entry_kind: EntryKind::DirectFiles,
        architecture_id: Some(focus.id.clone()), node_kind: Some(focus.kind), file_path: None,
        file_count: focus.direct_files.len(), description: Some("Files mapped directly to this focus, rather than an authored child.".into()),
        interfaces: Vec::new(), outside_focus: false, violation_rule_ids: Vec::new() }
}
...
fn representative(ir: &ArchitectureIr, focus: &CompiledNode, owner: &str, file: Option<&str>) -> Result<ProjectionNode> {
    if !is_within(owner, &focus.id) {
        let outside = ir.nodes.get(owner).with_context(|| format!("IR edge references missing node `{owner}`"))?;
    ...
    if let Some(child) = immediate_child(owner, &focus.id) {
        let child = ir.nodes.get(child).context("IR immediate child is missing")?;
...
impl ViolationIndex {
    fn new(violations: &[Violation]) -> Self {
        ...
        for violation in violations {
            let table = if violation.kind == "no_cycles" { &mut result.subtrees } else { &mut result.exact };
            for edge in &violation.architecture_edges {
                table.entry((edge.from.clone(), edge.to.clone(), edge.kind.clone())).or_default().insert(violation.rule_id.clone());
            }
        }
        result
    }
    fn matching(&self, edge: &ResolvedEdge) -> BTreeSet<String> {
        ...
        while let Some(a) = from {
            ...
            while let Some(b) = to {
                if let Some(ids) = self.subtrees.get(&(a.into(), b.into(), edge.kind.clone())) { found.extend(ids.iter().cloned()); }
                to = parent_id(b);
            }
            from = parent_id(a);
        }
...
pub fn project(ir: &ArchitectureIr, focus_id: &str, evidence_limit: usize) -> Result<Projection> {
    ...
    let all_violations = if evidence_limit == EVIDENCE_LIMIT { ir.violations.clone() }
        else { rules::evaluate(&ir.rules, &ir.resolved_edges, evidence_limit) };
    ...
    let mut groups: BTreeMap<(String, String, String, EdgeOrigin), ProjectedAccumulator> = BTreeMap::new();
    for edge in &ir.resolved_edges {
        if !is_within(&edge.from, focus_id) && !is_within(&edge.to, focus_id) { continue; }
        ...
        let rule_ids = rule_cache.entry((edge.from.clone(), edge.to.clone(), edge.kind.clone()))
        ...
        let accumulator = groups.entry((from_id, to_id, edge.kind.clone(), EdgeOrigin::Observed)).or_default();
        accumulator.observed.add(&edge.evidence, evidence_limit);
        accumulator.violations.extend(rule_ids.iter().cloned());
    }
    for edge in ir.edges.iter().filter(|edge| edge.origin == EdgeOrigin::Manual) {
        if !is_within(&edge.from, focus_id) && !is_within(&edge.to, focus_id) { continue; }
        ...
        if from_id == to_id { continue; }
        groups.entry((from_id, to_id, edge.kind.clone(), EdgeOrigin::Manual)).or_default().manual.extend(edge.manual_edges.iter().cloned());
    }
    let violations: Vec<Violation> = all_violations.into_iter().filter(|violation| {
        rules::violation_touches(violation, focus_id) || entries.values().any(|entry| {
            entry.architecture_id.as_deref().is_some_and(|id| rules::violation_touches(violation, id))
        })
    }).collect();
    for entry in entries.values_mut() {
        if entry.entry_kind == EntryKind::Architecture || entry.entry_kind == EntryKind::Boundary {
            if let Some(id) = &entry.architecture_id {
                for violation in &violations {
                    if rules::violation_touches(violation, id) { entry.violation_rule_ids.push(violation.rule_id.clone()); }
                }
            }
        }
    ...
    let edges = groups.into_iter().map(|((from, to, kind, origin), mut aggregate)| {
        aggregate.manual.sort_by(|a, b| a.id.cmp(&b.id));
        let mut edge = aggregate.observed.into_edge(from, to, kind);
        edge.origin = origin;
        if origin == EdgeOrigin::Manual { edge.count = aggregate.manual.len(); }
        edge.manual_edges = aggregate.manual;
        ProjectionEdge { edge, violation_rule_ids: aggregate.violations.into_iter().collect() }
    }).collect();
    let mut breadcrumbs = Vec::new();
    let mut current = Some(focus_id);
    while let Some(id) = current {
        breadcrumbs.push(NodeSummary::from(ir.nodes.get(id).context("IR breadcrumb ancestor is missing")?));
    ...
    Ok(Projection { focus: focus.clone(), breadcrumbs, nodes, edges, violations,
        evidence_limit, evidence_notice: ir.evidence_notice.clone() })
}
