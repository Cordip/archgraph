...
pub fn render(projection: &Projection) -> String {
    ...
    let ids: BTreeMap<&str, String> = projection.nodes.iter().enumerate().map(|(index, node)| (node.id.as_str(), format!("n{index}"))).collect();
    for node in &projection.nodes {
        let mut title = node.title.clone();
        if node.outside_focus { title.push_str(" (outside)"); }
        if !node.violation_rule_ids.is_empty() { title.push_str(" [VIOLATION]"); }
    ...
    for projected in &projection.edges {
        ...
        if edge.origin == EdgeOrigin::Manual { text.push_str(" manual"); }
        if !projected.violation_rule_ids.is_empty() { text.push_str(" VIOLATION"); }
        let arrow = if edge.origin == EdgeOrigin::Manual { "-.->" } else { "-->" };
        let _ = writeln!(out, "  {} {}|\"{}\"| {}", ids[edge.from.as_str()], arrow, label(&text), ids[edge.to.as_str()]);
    }
    out
}
