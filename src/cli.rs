use crate::{compiler, config, context, model::{ArchitectureIr, Violation, EVIDENCE_LIMIT}, paths::locate_repository,
    projection, provider::gitnexus::GitNexusCliProvider, render, rules::violation_touches, server};
use anyhow::{bail, Context, Result};
use clap::{Parser, Subcommand, ValueEnum};
use std::{net::IpAddr, path::{Path, PathBuf}};

#[derive(Debug, Parser)]
#[command(name = "archgraph", version, about = "Compile and check recursive architecture over GitNexus",
...
pub enum Commands {
    ...
    Show {
        node: Option<String>,
        #[arg(long, value_enum, default_value = "text")] format: ShowFormat,
    },
    /// Compile fresh and emit agent-oriented architecture context.
    Context {
        node: String,
        #[arg(long)] json: bool,
        #[arg(long, default_value_t = EVIDENCE_LIMIT)] evidence_limit: usize,
    },
    ...
    Serve {
        ...
        #[arg(long, default_value = "127.0.0.1")] host: IpAddr,
        #[arg(long, default_value_t = 7331)] port: u16,
    },
}

#[derive(Debug, Clone, Copy, ValueEnum)]
pub enum ShowFormat { Text, Json, Mermaid }

pub fn check_exit_code(violation_count: usize) -> u8 { if violation_count == 0 { 0 } else { 2 } }

pub fn matching_violations<'a>(ir: &'a ArchitectureIr, node: Option<&str>) -> Result<Vec<&'a Violation>> {
    if let Some(id) = node {
        if !ir.nodes.contains_key(id) { bail!("unknown architecture node `{id}`; inspect `archgraph show` or use UI search"); }
    }
    Ok(ir.violations.iter().filter(|violation| node.map_or(true, |id| violation_touches(violation, id))).collect())
}

pub async fn run(cli: Cli) -> Result<u8> {
    let cwd = std::env::current_dir().context("cannot read current directory")?;
    let root = locate_repository(cli.root.as_deref(), &cwd)?;
    let config_path = if cli.config.is_absolute() { cli.config.clone() } else { root.join(&cli.config) };
    if let Commands::Init { install_skill, force } = &cli.command {
    ...
    if let Some(id) = requested_node {
        if !validated.config.nodes.contains_key(id) { bail!("unknown architecture node `{id}`; inspect architecture.yaml or use UI search"); }
    }
    let provider = GitNexusCliProvider::new(&root, &validated.config.provider)?;
    let reindex = match &cli.command {
        Commands::Compile { reindex, .. } | Commands::Check { reindex, .. } | Commands::Serve { reindex, .. } => *reindex,
        _ => false,
    };
    let ir = compiler::compile(&root, &validated, &provider, reindex).await?;
    let ir_path = compiler::persist(&root, &ir)?;
    for warning in &ir.diagnostics.warnings { eprintln!("warning: {warning}"); }
    match cli.command {
        Commands::Compile { json, .. } => {
            if json {
                println!("{}", render::json::render(&serde_json::json!({
        ...
        Commands::Check { node, json, .. } => {
            ...
            if json {
                println!("{}", render::json::render(&serde_json::json!({"node": node, "violation_count": violations.len(),
                ...
                for violation in violations {
                    ...
                    for edge in &violation.architecture_edges {
                        println!("  {} -> {} [{}] × {}", edge.from, edge.to, edge.kind, edge.count);
                        for evidence in &edge.evidence {
                            println!("    {} -> {}", evidence.from_file, evidence.to_file);
                        }
                        if edge.evidence.len() < edge.count { println!("    (showing {} of {} observations)", edge.evidence.len(), edge.count); }
                    }
                }
                println!("\n{}", ir.evidence_notice);
            }
        ...
        Commands::Context { node, json, evidence_limit } => {
            let result = context::build(&ir, &node, evidence_limit)?;
            if json { println!("{}", render::json::render(&result)?); }
            else { print!("{}", context::markdown(&result)); }
            Ok(0)
        }
        Commands::Serve { host, port, .. } => {
            if !host.is_loopback() { eprintln!("warning: binding to {host} exposes read-only architecture metadata to reachable clients; there is no authentication"); }
...
pub fn initialize(root: &Path, config_path: &Path, install_skill: bool, force: bool) -> Result<()> {
    if config_path.exists() && !force {
        ...
        std::fs::write(config_path, STARTER).with_context(|| format!("cannot write {}", config_path.display()))?;
        println!("Created {}", config_path.display());
    }
    if install_skill {
        let mut supported: Vec<PathBuf> = [".claude", ".agents"].into_iter()
            .map(|name| root.join(name)).filter(|path| path.is_dir()).collect();
        // No existing agent directory: use the project-local, vendor-neutral default.
        if supported.is_empty() { supported.push(root.join(".agents")); }
        for agent in supported {
            ...
            for ancestor in directory.ancestors().take_while(|path| path.starts_with(root)) {
            ...
            if destination.exists() && !force {
                println!("Keeping existing {} (use --force to overwrite)", destination.display());
                continue;
            }
            if destination.symlink_metadata().is_ok_and(|metadata| metadata.file_type().is_symlink()) {
                bail!("refusing to overwrite symlink {}; replace it with a project-local skill file", destination.display());
            }
            std::fs::write(&destination, SKILL).with_context(|| format!("cannot install skill {}", destination.display()))?;
...
mod tests {
    use super::*;
    #[test]
    fn check_exit_semantics() { assert_eq!(check_exit_code(0), 0); assert_eq!(check_exit_code(1), 2); }
    #[test]
    fn all_commands_parse_and_global_flags_work_after_subcommand() {
        for args in [vec!["archgraph", "init", "--install-skill"], vec!["archgraph", "compile", "--reindex"],
            vec!["archgraph", "check", "app.a", "--json"], vec!["archgraph", "show", "--format", "mermaid"],
            vec!["archgraph", "context", "app", "--evidence-limit", "50"], vec!["archgraph", "serve", "--root", "/tmp"]] {
