...
pub async fn serve(ir: ArchitectureIr, host: IpAddr, port: u16) -> Result<()> {
    ...
    eprintln!(
        "ArchGraph: http://{} (read-only snapshot; Ctrl-C to stop)",
        listener.local_addr()?
    );
    axum::serve(listener, router(Arc::new(ir)))
...
async fn focus(
    State(ir): State<Arc<ArchitectureIr>>,
    Path(id): Path<String>,
...
async fn violations(State(ir): State<Arc<ArchitectureIr>>) -> Json<Value> {
